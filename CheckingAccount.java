...
//BankAccountv2: CheckingAccountv2 -- redo
...
//-----------------------constrs-----------------------   
   ...
   CheckingAccount(boolean accountActivePassed, double balancePassed, double annualInterestRatePassed,
   String accountTypePassed)
   {
      super(balancePassed,.01, accountTypePassed);
...
//-----------------------other meths-------------------
   ...
   {
      if(getBalance() > 10000)
         setBalance( getBalance() + (getBalance() * .01) );
   }//end calcInterest
   ...
   public void monthlyProcess()
   {
      ...
      if (calendar.getDate() == 28 )
      {
         ...
         if (getMonthlyWithdrawals() > 10)
            setServiceCharges(getMonthlyWithdrawals() - 10);
         System.out.println("Service Charges : -$" + getServiceCharges() );
         setBalance(getBalance() - getServiceCharges() );
         System.out.println(getAccountType() + " interest rate (for balances over $10,000: %" + (getAnnualInterestRate() * 100) ); 
         calcInterest();
